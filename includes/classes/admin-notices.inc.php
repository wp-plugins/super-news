...
/**/
if (!class_exists ("c_ws_plugin__super_news_admin_notices"))
	{
		class c_ws_plugin__super_news_admin_notices
			{
				/*
						...
						if ($notice && is_string ($notice)) /* If we have a valid string. */
							{
								...
								unset ($__refs, $__v); /* Unset defined __refs, __v. */
								/**/
								update_option ("ws_plugin__super_news_notices", c_ws_plugin__super_news_utils_arrays::array_unique ($notices));
							}
						/**/
														...
														if (strtotime ("now") >= (int)$notice["time"]) /* Time to show it? */
															{
																...
																/**/
																if (!$notice["dismiss"] || $_GET["ws-plugin--super-news-dismiss-admin-notice"] !== md5 ($notice["notice"]))
																	c_ws_plugin__super_news_admin_notices::display_admin_notice ($notice["notice"],$notice["error"],$notice["dismiss"]);
																/**/
