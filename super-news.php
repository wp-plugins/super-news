...
Define versions.
*/
define ("WS_PLUGIN__SUPER_NEWS_VERSION", "1.0.5"); /* Since 1.0.5. */
define ("WS_PLUGIN__SUPER_NEWS_MIN_PHP_VERSION", "5.2");
define ("WS_PLUGIN__SUPER_NEWS_MIN_WP_VERSION", "2.9.2");
...
if (version_compare (PHP_VERSION, WS_PLUGIN__SUPER_NEWS_MIN_PHP_VERSION, ">=") && version_compare (get_bloginfo ("version"), WS_PLUGIN__SUPER_NEWS_MIN_WP_VERSION, ">=") && !isset ($GLOBALS["WS_PLUGIN__"]["super_news"]))
...
else if (is_admin ()) /* Admin compatibility errors. */
	{
		if (!version_compare (PHP_VERSION, WS_PLUGIN__SUPER_NEWS_MIN_PHP_VERSION, ">="))
			{
				add_action ("admin_notices", create_function ('', 'echo \'<div class="error fade"><p>You need PHP v\' . WS_PLUGIN__SUPER_NEWS_MIN_PHP_VERSION . \'+ to use the Super News plugin.</p></div>\';'));
			}
		else if (!version_compare (get_bloginfo ("version"), WS_PLUGIN__SUPER_NEWS_MIN_WP_VERSION, ">="))
			{
				add_action ("admin_notices", create_function ('', 'echo \'<div class="error fade"><p>You need WordPress® v\' . WS_PLUGIN__SUPER_NEWS_MIN_WP_VERSION . \'+ to use the Super News plugin.</p></div>\';'));
			}
	}
?>