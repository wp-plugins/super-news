...
*/
else if (is_admin ()) /* Admin compatibility errors. */
	{
		if (!version_compare (PHP_VERSION, "5.2", ">="))
			{
				add_action ("admin_notices", create_function ('', 'echo \'<div class="error fade"><p>You need PHP version 5.2 or higher to use the Super News plugin.</p></div>\';'));
			}
		else if (!version_compare (get_bloginfo ("version"), "2.9.2", ">="))
			{
				add_action ("admin_notices", create_function ('', 'echo \'<div class="error fade"><p>You need WordPress® 2.9.2 or higher to use the Super News plugin.</p></div>\';'));
			}
	}
?>